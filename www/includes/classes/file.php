...
/* 		public function get_name(){
		...
		public function upload_file(){
			...
			$request_upload_file->execute(array(':content_title' => $content_title, ':content_slug' => $content_slug, ':content_author_id' => $content_author_id, ':content_type' => $content_type, ':content_status' => $content_status, ':content_parent_id' => $content_parent_id, ':content_has_children' => $content_has_children, ':content_content' => $content_content));
			$request_upload_file->closeCursor();
		}

		public function update_file(){
			global $DDB;
			$content_update_request = $DDB -> prepare('UPDATE at_content SET content_title = :content_title, content_slug = :content_slug, content_author_id = :content_author_id, content_type = :content_type, content_status = :content_status, content_parent_id = :content_parent_id, content_has_children = :content_has_children, content_content = :content_content WHERE content_id = :content_id');
			$content_update_request -> execute(array(':content_title' => $this->file_title, ':content_slug' => $content_slug, ':content_author_id' => $this->file_author_id, ':content_type' => $this->file_type, ':content_status' => $this->file_status, ':content_parent_id' => $this->file_parent_id, ':content_has_children' => $this->file_has_children, ':content_content' => $this->file_content, ':content_id' => $this->file_id));
